...
export default function CitizenReport() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Single POST request
      const res = await api.post('/case', formData, { 
        headers: { 'Content-Type': 'multipart/form-data' } 
      });
  
      // SAVE TO LOCAL CACHE using the ID returned by the server
      if (res.data && res.data.id && imagePreview) {
        useImageCache.getState().saveImage(res.data.id, imagePreview);
      }
  ...
  if (isSuccess) {
    return (
      <div className="flex flex-col items-center justify-center h-screen bg-[#2A835F] p-6 text-center font-sans">
  ...
  return (
    <div className="min-h-screen bg-[#f0f6f4] flex flex-col pb-24 font-sans">
      ...
      <main className="flex-1 px-4 max-w-md w-full mx-auto -mt-20 relative z-10 space-y-5">
        ...
        {activeTab === 'new' && (
          <div className="space-y-4 animate-in fade-in slide-in-from-bottom-8">
            <div className="bg-white rounded-3xl shadow-xl p-5 border border-white">
              ...
              {imagePreview ? (
                <div className="relative rounded-2xl overflow-hidden h-56 border-4 border-[#2A835F] shadow-md">
                  {/* For a fresh preview, we just use the local imagePreview string */}
            ...
            <div className="bg-white rounded-3xl shadow-xl p-5 border border-white">
              <h2 className="font-bold text-gray-800 mb-3 flex items-center gap-2 text-sm">
                <div className="bg-gray-200 text-gray-600 w-6 h-6 rounded-full flex items-center justify-center text-xs font-black shadow-inner">3</div> Details (Optional)
              </h2>
            ...
            <button onClick={handleSubmit} disabled={!imageFile || !location || isSubmitting}
              className={`w-full py-5 rounded-2xl font-black text-lg shadow-2xl flex items-center justify-center gap-2 transition-all mt-6 ${
                imageFile && location && !isSubmitting ? 'bg-[#2A835F] text-white hover:bg-[#226a4c] hover:-translate-y-1' : 'bg-gray-200 text-gray-400'
              }`}>
              ...
              myReports.map((report) => (
                <div key={report.id} className="bg-white rounded-3xl shadow-lg border border-white p-3 flex gap-4 items-center">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-gray-500 mt-1 mb-2 font-medium">{new Date(report.createdAt).toLocaleDateString()}</p>
      ...
      <nav className="fixed bottom-6 left-0 w-full z-50 px-6 font-sans">
        <div className="bg-gray-900 text-white rounded-full flex justify-between max-w-sm mx-auto p-2 shadow-2xl">
